...
def test_full_review_posts_inline_and_summary(provider, state_db, pr, repo_config, global_config, project_config):
...
def test_re_review_deletes_old_comments_first(provider, state_db, pr, repo_config, global_config, project_config):
...
def test_duplicate_findings_deduplicated(provider, state_db, pr, repo_config, global_config, project_config):
...
def test_skip_severities_filtered(provider, state_db, pr, repo_config, global_config):
...
def test_dry_run_posts_nothing(provider, state_db, pr, repo_config, global_config, project_config, capsys):
    """Dry run prints output but makes no API calls."""
    result = make_result([make_finding()])

    post_review(provider, state_db, pr, result, repo_config, project_config, global_config, dry_run=True)
